...
function LoginPage() {
  ...
  const { data: unclaimSupply } = useDropUnclaimedSupply(program);
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center text-center bg-[#F5AB05]">
      ...
      <main className="z-30 text-white">
        <h1 className="text-4xl font-bold uppercase">
          Welcome to the <span className="text-fuchsia-600">Test NFT</span>
        </h1>
        {!user && (
          <div>
            <button
        ...
        {user && (
          <div>
            ...
            {isLoading && (
              <div className="text-2xl font-bold mb-5 bg-fuchsia-600 text-white py4 px-10 border-2 border-fusbg-fuchsia-600 animate-pulse rounded-md transition duration-200">
                Hold on, We are just looking for your membership pass...
              </div>
            )}
            {usersNft && (
              <Link
            ...
            {!usersNft &&
              ...
              (unclaimSupply && unclaimSupply > 0 ? (
                ...
                <p className="text-2xl font-bold mb-5 bg-red-500 text-white py-4 px-10 border-2 border-red-500 rounded-md uppercase transition duration-200">
                  Sorry, we are all out of Membership Pass
                </p>
              ))}
          </div>
        )}
